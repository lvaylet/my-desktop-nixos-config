...
3. A declarative Home Manager module (`modules/home-manager/antigravity.nix`) provisioning the latest Antigravity CLI and pre-configuring environment variables (`ANTIGRAVITY_API_BASE=http://127.0.0.1:11434/v1`, `ANTIGRAVITY_MODEL=gemma4:12b`, `ANTIGRAVITY_OFFLINE=1`).
4. Host assembly updates in `machines/desktop-pc/configuration.nix` and task runner verification recipes in `justfile`.

## Technical Context

**Language/Version**: Nix Expression Language (2.18+ / 2.24+ Flakes), Shell / Bash

**Primary Dependencies**: `services.ollama` (NixOS module with `pkgs.ollama-cuda`), `antigravity-cli`, `pkgs.linuxPackages_latest.nvidiaPackages.latest`
...
**Performance Goals**: 100% GPU layer offloading into 16 GB VRAM; generation latency <2s time-to-first-token; ~40+ tokens/sec sustained throughput; ~2–3 GB VRAM reserved for desktop compositing; fast Nix rebuild times
...
```text
.
├── justfile                                   # Operational task runner recipes (e.g. download-model, ollama-status)
├── machines/
│   └── desktop-pc/
...
- Extends `justfile` with operational health, model download (`just download-model`), and diagnostic recipes.
